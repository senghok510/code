...
class IMDBBertDataset(Dataset):
    ...
    def __init__(self, path, ds_from=None, ds_to=None, should_include_text=False):
        ...
        self.tokenizer = get_tokenizer('basic_english')  # normalized + space split
...
# 1. 
    ...
    def prepare_dataset(self) -> pd.DataFrame:
        ...
        # ds: raw imported dataset
        for review in self.ds:
            review_sentences = review.split('. ')
            sentences += review_sentences
            self._update_length(review_sentences, sentence_lens)
        # length that separate 70% of the total lengths
    ...
    def _update_length(self, sentences: List[str], lengths: List[int]):
        for v in sentences:
            l = len(v.split())  # length of the sentence
            lengths.append(l)
        return lengths
    ...
    def _fill_vocab(self):
        ...
        self.vocab.insert_token(self.CLS, 0) # label to id
        ...
        self.vocab.set_default_index(4)  # other than the designated above
    # 
    def _create_item(self, first: List[str], second: List[str], target: int = 1):
        ...
        nsp_sentence = updated_first + [self.SEP] + updated_second  # NSP
